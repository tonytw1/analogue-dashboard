...
def sendValues(filepath,zabbixserver="localhost",zabbixport=10051):
    print filepath
    print "zabbix_sender --zabbix-server '%s' --port '%s' -i '%s' -vv"%(zabbixserver,zabbixport,filepath)
    r=os.system("zabbix_sender --zabbix-server '%s' --port '%s' -i '%s' -vv"%(zabbixserver,zabbixport,filepath))
    if r!=0:
...
License: GPLv2
    ...
    try:
        ...
        try:
            sendValues(filepath=tempfilepath,zabbixserver=opts.zabbixserver,zabbixport=opts.zabbixport)
